...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 30px Figtree', fontUrl, scrollSpeed = 2, scrollEase = 0.05 }) {
  ...
  useEffect(() => {
    if (!containerRef.current) { console.log('[CircularGallery] no container ref'); return; }
    console.log('[CircularGallery] container found, size:', containerRef.current.clientWidth + 'x' + containerRef.current.clientHeight);
    let app;
    let isMounted = true;
    resolveFont(font, fontUrl).then(resolvedFont => {
      if (!isMounted || !containerRef.current) { console.log('[CircularGallery] unmounted during font load'); return; }
      console.log('[CircularGallery] font resolved, creating App');
      try {
        app = new App(containerRef.current, { items, bend, textColor, borderRadius, font: resolvedFont, scrollSpeed, scrollEase });
        console.log('[CircularGallery] App created successfully');
      } catch (err) {
        console.error('[CircularGallery] App creation failed:', err);
      }
    });
    return () => { isMounted = false; if (app) app.destroy(); };
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase]);
  return h('div', { className: 'circular-gallery', ref: containerRef, tabIndex: 0, role: 'region', 'aria-label': 'Circular image gallery. Use left and right arrow keys to navigate.' });
}
