import React, { useEffect, useId, useLayoutEffect, useRef, useState } from 'https://esm.sh/react@18.3.1';
import { HugeiconsIcon } from 'https://esm.sh/@hugeicons/react@1.1.10?deps=react@18.3.1';
import { ArrowDown01Icon, Tick02Icon } from 'https://esm.sh/@hugeicons/core-free-icons@4.3.3?deps=react@18.3.1';
...
export default function GlideSelect({
  ...
  return h(
    ...
    h(
      ...
      h(
        'span',
        { className: 'glide-select__chevron', 'aria-hidden': 'true' },
        h(HugeiconsIcon, { icon: ArrowDown01Icon, size: 12, strokeWidth: 2.5 })
      )
    ),
    phase !== 'closed'
      ? h(
          ...
          h(
            ...
            h('span', { ref: pillRef, className: 'glide-select__pill', 'aria-hidden': 'true' }),
            items.map((it, i) =>
              h(
                'div',
                {
                ...
                h('span', { className: 'glide-select__name' }, it.label),
                showTags && it.tag ? h('span', { className: 'glide-select__tag' }, it.tag) : null,
                h(
                  'span',
                  {
                  ...
                  h(HugeiconsIcon, { icon: Tick02Icon, size: 13, strokeWidth: 2.5 })
                )
              )
            )
          )
